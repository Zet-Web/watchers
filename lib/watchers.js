...
function addMegaServer(options){
	...
	megaServer.bind('subscribe', function(clientId) {
		...
		megaServerTrans.publish('/'+serverDefaultChannel,  {port:port, init:true, server_version:"v0.3.32"});
	});
	...
	megaServerTrans.subscribe('/'+serverDefaultChannel+"toServer", function(message) {
		...
		if (message.action === 'changeInject'){
			changeInject(message.port);
		}
	});
...
function changeInject(port){
	if (true){
		console.log('\n______________ changeInject [external]: ' + port);
	}
	if (port >0){
		for (var i = serverWatchers.length - 1; i >= 0; i--) {
			if (!true){
				console.log(serverWatchers[i])
				console.log(JSON.stringify(serverWatchers[i]))
				console.log(serverWatchers[i].inject());
				serverWatchers[i].changeInject(true);
				console.log(serverWatchers[i].inject());
			}
		};
	}
...
function serverWatcher(options){
	...
	//SET
	this.changeInject = function(val){inject = val};
	...
	var lazysendChanges  = _.debounce(sendChanges, 250, false);
