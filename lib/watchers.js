...
function addMegaServer(options){
	...
	megaServer.bind('subscribe', function(clientId) {
		...
		megaServerTrans.publish('/'+serverDefaultChannel,  {port:port, init:true, server_version:"v0.3.26"});
	});
...
function serverWatcher(options){
	...
	FayeSenderListener.publish('/'+defaultChannel,  {ver:versionChange, port:port, injectcss:null, refresh:false, init:true, test:"OK__________-----1!", name:lastName});
	...
	server.bind('subscribe', function(clientId) {
		...
		FayeSenderListener.publish('/'+defaultChannel,  {ver:versionChange, port:port, injectcss:null, refresh:false, test:"OK__________-----!2", init:true, name:lastName});
	});
