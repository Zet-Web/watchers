                ...
                files.forEach(function (f, index) {
                    ...
                    fs.stat(f, function (err, stat) {
                        ...
                        if (!enoent) {
                            ...
                            /*KRC*/
                            if (stat !== undefined) {
                                if (!options.filter || !options.filter(f, stat, path.basename(f))) {
                                    if (!true) console.log('     Watch: ' + f);
...
exports.watchTree = function (root, options, callback) {
    ...
    walk(root, options, function (err, files) {
        ...
        var fileWatcher = function (f) {
            ...
            /*KRC No dispara en nuevos files Valida con el array*/
            for (var i = validFiles.length - 1; i >= 0; i--) {
                if (f == validFiles[i]) {
                    console.log('		Watch: ' + f);
